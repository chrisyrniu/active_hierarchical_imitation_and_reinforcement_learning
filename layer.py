...
class Layer():
    ...
    def choose_action(self,agent, env, subgoal_test):
        ...
        if agent.FLAGS.test or subgoal_test or self.layer_number == 0:
    ...
    def train(self, agent, env, subgoal_test = False, episode_num = None):
        ...
        while True:
            ...
            # If layer is bottom level, execute low-level action
                ...
                ##@@
                if agent.FLAGS.show:
                    agent.worker.agentMove(next_state[:2])
                    for i in range(len(agent.goal_array)):
                        agent.worker.goalChange(np.array(agent.goal_array[i][:2]),i)
                    agent.worker.posChange()
                ##@@
