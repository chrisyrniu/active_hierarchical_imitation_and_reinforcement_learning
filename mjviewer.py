...
class MjViewerBasic(cymj.MjRenderContextWindow):
    ...
    def _mouse_button_callback(self, window, button, act, mods):
        ...
        print("build mujoco_py")
        print((self._last_mouse_x, self._last_mouse_y))
        self.actions.add((self._last_mouse_x, self._last_mouse_y))
...
class MjViewer(MjViewerBasic):
    ...
    def key_callback(self, window, key, scancode, action, mods):
        if key == glfw.KEY_RIGHT:
            rgba_len = len(self.sim.model.site_rgba)
            self.sim.data.mocap_pos[rgba_len-1][0] += 0.1
            if self.agent_wait:
                print(self.sim.data.mocap_pos[rgba_len-1])
            ...
            rgba_len = len(self.sim.model.site_rgba)
            self.sim.data.mocap_pos[rgba_len-1][1] += 0.1
            if self.agent_wait:
                print(self.sim.data.mocap_pos[rgba_len-1])
